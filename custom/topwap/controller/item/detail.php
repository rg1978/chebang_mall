...
class topwap_ctl_item_detail extends topwap_controller {
    ...
    public function index()
    {
        ...
        //获取优惠券信息
		$objMdlCoupon = app::get('syspromotion')->model('coupon_item');
        $couponDetail = $objMdlCoupon->getRow('*',array('item_id'=>$itemId));
        if($couponDetail)
        {
			$couponData = app::get('syspromotion')->model('coupon')->getRow('coupon_name',array('coupon_id'=>$couponDetail['coupon_id']));
			$couponDetail['promotion_name'] = $couponData['coupon_name'];
            $pagedata['couponDetail'] = $couponDetail;
        }
		
		//获取组合促销信息
		$objMdlPackage = app::get('syspromotion')->model('package_item');
        $packageDetail = $objMdlPackage->getRow('*',array('item_id'=>$itemId));
        if($packageDetail)
        {
			$packageData = app::get('syspromotion')->model('package')->getRow('package_name',array('package_id'=>$packageData['package_id']));
			$packageDetail['promotion_name'] = $packageData['package_name'];
            $pagedata['packageDetail'] = $packageDetail;
        }
		
		//获取赠品促销信息
		$objMdlGift = app::get('syspromotion')->model('gift_item');
        $giftDetail = $objMdlGift->getRow('*',array('item_id'=>$itemId));
        if($giftDetail)
        {
			$giftData = app::get('syspromotion')->model('gift')->getRow('gift_name',array('gift_id'=>$giftData['gift_id']));
			$giftDetail['promotion_name'] = $giftData['gift_name'];
            $pagedata['giftDetail'] = $giftDetail;
        }

        //判定促销信息是否存在
        if($pagedata['couponDetail']||$pagedata['packageDetail']||$pagedata['giftDetail']||$pagedata['promotionDetail']){
            $pagedata['ispromotion'] = 1;
        }else{
            $pagedata['ispromotion'] = 0;
        }
        ...
        // 获取评价列表
        $pagedata['rateList'] = $this->__searchRate($itemId);
        // 获取当前平台设置的货币符号和精度
