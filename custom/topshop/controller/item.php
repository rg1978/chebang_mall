...
class topshop_ctl_item extends topshop_controller {
	...
	public function addService() 
	{
        ...
        $dlytmpls = app::get('topshop')->rpcCall('logistics.dlytmpl.get.list',$tmpParams);
        $pagedata['dlytmpls'] = current($dlytmpls['data']);
        $this->contentHeaderTitle = app::get('topshop')->_('添加商品（服务类）');
        return $this->page('topshop/item/edit_service.html', $pagedata);
	}
    ...
    public function edit()
    {
        ...
        if ($pagedata['item']['item_type'] == 1)
        {
	        $pagedata['dlytmpls']['template_id'] = $pagedata['item']['dlytmpl_id'];
   	        $this->contentHeaderTitle = app::get('topshop')->_('编辑商品（服务类）');
	        return $this->page('topshop/item/edit_service.html', $pagedata);
        }
        else
        {
...
//echo '<pre>';print_r($pagedata);exit();
	...
	// 添加属性值
	public function addPrpoSpec()
	{
		$pagedata['prop_id'] = input::get('prop_id');
		return $this->page('topshop/item/props/addspec.html', $pagedata);
	}

	// 保存属性值
	public function savePrpoSpec()
	{
		$postData = input::get();
        try
        {
            //$result = app::get('topshop')->rpcCall('shop.update',$postData);
            $mdlPropValues = app::get('syscategory')->model('prop_values');
            ...
            $result = $mdlPropValues->save($postData);
            if( $result )
            {
                $msg = app::get('topshop')->_('保存属性值成功，请等待平台审核。');
                $result = 'success';
            }
            else
            {
                $msg = app::get('topshop')->_('保存属性值失败');
                $result = 'error';
            }
        }
        catch(Exception $e)
        ...
        $this->sellerlog('申请属性值，属性值名：' .$postData['prop_value']);
        $url = url::action('topshop_ctl_item@add');
