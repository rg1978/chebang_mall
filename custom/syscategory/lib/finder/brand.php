...
class syscategory_finder_brand {
    ...
    public function column_edit(&$colList, $list){
        ...
        {
			// 2016-12-23 zhoumin  新增审核操作
			$result = "";
			if ($row['status'] != 'successful')
			{
				$url = '?app=syscategory&ctl=admin_brand&act=doExamine&_finder[finder_id]='.$_GET['_finder']['finder_id'].'&p[0]='.$row['brand_id'];
				$target = 'dialog::  {title:\''.app::get('syscategory')->_('审核').'\', width:500, height:600}';
            ...
            $result .= '<a href="' . $url . '" target="' . $target . '">' . app::get('syscategory')->_('编辑') . '</a>';
	        $colList[$k] = $result;   
        }
    }
