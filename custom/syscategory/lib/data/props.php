...
class syscategory_data_props {
    ...
    public function add($data, &$msg)
    {
        ...
        $flag = $this->propModel->savePropValues($data['prop_value'], $data['status'], $propId, $msg);
        //-----------------------------------------------------------------------------
    ...
    public function getNatureProps($catId)
    {
        ...
        $prePropIds = array_column($result,'prop_id');
        if( !$prePropIds) return array();

        $objMdlPropValues = app::get('syscategory')->model('prop_values');
        $propValues =  $objMdlPropValues->getList('*',array('prop_id'=>$prePropIds));
        foreach( (array)$propValues as $row )
        {
            $propValueData[$row['prop_id']][] = $row;
        }

        foreach($result as &$v)
        {
            $v['prop_value'] = $propValueData[$v['prop_id']];
        }
    ...
    public function getPropsValueList($propValueId, $fields='*')
    {
        ...
        $objMdlPropValues = app::get('syscategory')->model('prop_values');
        $propValuesList = $objMdlPropValues->getList($fields, array('prop_value_id'=>$propValueId, 'status' => 'successful') );
