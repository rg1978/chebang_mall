...
class syscategory_mdl_props extends dbeav_model{
	...
	public function savePropValues($propValue, $status, $propId, &$msg)
	//-------------------------------------------------------------------------
    {
    ...
    public function modifier_prop_type(&$colList)
    {
        if( $colList )
        {
            foreach($colList as $k=>$row)
            {
                if($row=='spec')
                {
                    $colList[$k] = '<label style="color:#3c763d;">销售属性</label>';
                }
                elseif($row=='nature')
                {
                    $colList[$k] = '自然属性';
