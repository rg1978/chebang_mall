...
class syscategory_ctl_admin_props extends desktop_controller {
    ...
    public function _views(){
        ...
        $sub_menu = array(
            ...
            1=>array('label'=>app::get('syscategory')->_('销售属性'),'optional'=>false,'filter'=>array('prop_type'=>array('spec'),'disabled'=>0)),
	...
	public function examineValues() 
	{
		return $this->finder(
            'syscategory_mdl_prop_values',
            array(
                'title'=>app::get('syscategory')->_('属性值审核'),
                'actions'=>array(),
                'use_view_tab' => false,
            )
        );
	...
	public function doExamineValues($prop_value_id)
	{

		$mdlValue = app::get('syscategory')->model('prop_values');
		$data = $mdlValue->getRow('prop_id,prop_value,shop_name,reason', array('prop_value_id' => $prop_value_id));
		$mdlProp = app::get('syscategory')->model('props');
		$dataProp = $mdlProp->getRow('prop_name', array('prop_id' => $data['prop_id']));
		$pagedata['prop_value'] = $data;
		$pagedata['prop'] = $dataProp;
	    return view::make('syscategory/admin/prop/examineValue.html', $pagedata);
    }

    public function saveExamineValues()
    {
	    $postdata = $_POST ;
        $this->begin();
        try{
            ...
            {
	            $postdata['agree_time'] = time();
            }
            else
            {
				$postdata['refuse_time'] = time();
            }
            $result = $mdlValue->update($postdata, array('prop_value_id' => $postdata['prop_value_id']));
        }
        ...
        $url_param = array('app'=>'syscategory', 'ctl'=>'admin_props', 'act'=>'examineValues');
