...
class syscategory_ctl_admin_brand extends desktop_controller {
	...
	// 2016-12-23 zhoumin 品牌审核
	// 审核页面
	public function doExamine($brandId)
	{
		$brandInfo = app::get('syscategory')->model('brand')->getBrandRow($brandId);
		$shopAccount = app::get('sysshop')->model('account');
		$account = $shopAccount->getRow('login_account', array('seller_id' => $brandInfo['seller_id']));
		$brandInfo['login_account'] = $account['login_account'];
		$pagedata['brandInfo'] = $brandInfo;
		return $this->page('syscategory/admin/brand/examine.html', $pagedata);
	}
	...
	public function saveExamine()
	{
        ...
        $this->begin();
        try{
            $brandInfo = app::get('syscategory')->model('brand');
            ...
            $result = $brandInfo->update($postdata, array('brand_id' => $postdata['brand_id']));

            $mdlSeller = app::get('sysshop')->model('seller');
            $sellerInfo = $mdlSeller->getRow('shop_id', array('seller_id' => $postdata['seller_id']));
            $mdlShopBrand = app::get('sysshop')->model('shop_rel_brand');
            $newRelBrand['shop_id'] = $sellerInfo['shop_id'];
            $newRelBrand['brand_id'] = $postdata['brand_id'];
            $mdlShopBrand->save($newRelBrand);
        }
        ...
        $url_param = array('app'=>'syscategory', 'ctl'=>'admin_brand', 'act'=>'index');
