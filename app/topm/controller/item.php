...
class topm_ctl_item extends topm_controller {
    ...
    public function index()
    {
        ...
        {
            return redirect::action('topc_ctl_item@index',array('item_id'=>$itemId))->send();
        }
        //相册图片
        ...
        //获取商品的促销信息
        $promotionInfo = app::get('topm')->rpcCall('item.promotion.get', array('item_id'=>$itemId));
        if($promotionInfo)
        {
            foreach($promotionInfo as $vp)
            {
                $basicPromotionInfo = app::get('topm')->rpcCall('promotion.promotion.get', array('promotion_id'=>$vp['promotion_id'], 'platform'=>'wap'));
        ...
        // 获取当前平台设置的货币符号
        $cur_symbol = app::get('topm')->rpcCall('currency.get.symbol',array());
