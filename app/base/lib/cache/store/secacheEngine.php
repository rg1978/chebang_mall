...
class base_cache_store_secacheEngine 
{
    ...
    protected function internalFetch($key, &$value)
    {
        // If the file doesn't exists, we obviously can't return the cache so we will
        // just return null. Otherwise, we'll get the contents of the file and get
        // the expiration UNIX timestamps from the start of the file's contents.
        try {
            if (false===$this->_fetch($key, $data)) throw new Exception('Secached fetch fail.');
        ...
        {
            $this->delete($key);
            $value = $value = ['content' => null, 'expiration' => null];
            return;
        }
    ...
    function _fetch($key,&$return, $isLock = true)
    {
        if($isLock)
        {
            if ($this->lock(false))
    ...
    function increment($key, $value =1, $initial = 0, $minutes)
    {
        if($this->lock(true))
        {
            $this->internalFetch($key, $origData, false);

            if ($origData['content'] === null) {
                $data = ['content' => $initial, 'expiration' => $this->expiration($minutes)];
            } else {
                $data = $origData;
            }
