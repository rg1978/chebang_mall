...
if( !function_exists('getQrcodeUri') )
{
    function getQrcodeUri($text, $size, $padding=0)
    {
        $qrCode = new QrCode();
        $size = $size ? : 60;
        $data = cache::store('qrcode')->get($text.$size);
        if( !$data )
        {
            $data = $qrCode
                ->setText($text)
            ...
            cache::store('qrcode')->forever($text.$size, $data);
        }
        return $data;
    }
}

if( !function_exists('parseSearchKeyWord') )
{
    //过滤CRLF注入攻击
