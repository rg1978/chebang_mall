...
class image_finder_image{
    ...
    public function column_img(&$colList, $list) {
        foreach($list as $k=>$row)
        {
            ...
            $imageUrl =  base_storager::modifier($row['url']);
            $colList[$k] = '<div  style="width:50px;height:50px;display:block;font-family:Arail;vertical-align: middle;display:table-cell;font-size:42.5px;padding:1px;background:#fff;"><a href="'.$imageUrl.'" target="_blank" style="display:block;">
<img src="'.$imageUrl.'" '.$size.' /></a></div>';
        }
    }
