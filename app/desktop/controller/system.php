...
class desktop_ctl_system extends desktop_controller{
    ...
    function set_title(){
        header("cache-control: no-store, no-cache, must-revalidate");
        $allsize = array();
        if( input::get('background_title') )
        {
            $this->app->setConf('background_title', input::get('background_title'));
        }
        $pagedata['background_title'] = $this->app->getConf('background_title');

        return $this->page('desktop/system/background_title.html', $pagedata);
