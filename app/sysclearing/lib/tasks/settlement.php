...
class sysclearing_tasks_settlement extends base_task_abstract implements base_interface_task
{
    ...
    public function exec($params=null)
    {
        ...
        $filter = array(
            ...
            'settlement_time|sthan'=>strtotime(date('Y-m-t  23:59:59', strtotime('-1 month'))),
        );
        ...
        foreach($shopids as $v)
        {
            ...
            if($settlementList =  $objMdlSettlementDetail->getList('*',$filter))
            {
                ...
                $settle['settlement_fee_amount'] = $objLibMath->number_plus([$settle['settlement_fee_amount'],$settle['post_fee_amount']]);
