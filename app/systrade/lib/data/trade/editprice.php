...
class systrade_data_trade_editprice
{
    ...
    public function generate($params)
    {
        ...
        $tradeInfo = $objTrade->getTradeInfo('payment,tid,status,shop_id,total_fee,discount_fee,adjust_fee,post_fee', array('tid'=>$params['tid']));
        $tradeInfo['orderPayment'] = $tradeInfo['payment']-$tradeInfo['post_fee'];
        ...
        $aDataTmp = array(
            'tid' => $params['tid'],
            'post_fee' => $params['post_fee'] ? $params['post_fee'] : 0,
        );
        $orderPayment = array();
        $orderAdjustFee = array();

        $aDataTmp['payment'] = $objMath->number_plus(array($tradeInfo['orderPayment'], $aDataTmp['post_fee']));
        ...
        //echo '<pre>';print_r($aDataTmp);exit();
        ...
        try
        {
            ...
            event::fire('trade.editPrice',[$params['tid'], $tradeInfo['shop_id'],$aDataTmp]);
