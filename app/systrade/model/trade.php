...
class systrade_mdl_trade extends dbeav_model{
    ...
    public function modifier_dlytmpl_ids(&$colList)
    {
        if( $colList )
        {
            $dlytmplIds = implode(',',$colList);
            $dlytmpls = app::get('systrade')->rpcCall('logistics.dlytmpl.get.list',['dlytmpl_ids'=>$dlytmplIds]);
            $dlytmplData = $dlytmpls['data'];
            foreach ($dlytmplData as $value) {
                foreach ($colList as $k => $val) {
                    if($value['template_id'] == $val)
                    {
                        $tmp[$val] = $value['name']; 
                        $colList[$k] = $tmp[$val]; 
                    }      
