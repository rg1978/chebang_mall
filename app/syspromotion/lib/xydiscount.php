    ...
    private function __preareData($data) {
        ...
        if($data['xydiscount_id'])
        {
            ...
            if(!app::get('sysconf')->getConf('shop.promotion.examine')){
                ...
                if($xydiscountInfo['xydiscount_status'] =='pending'){
                    throw new \LogicException('X件Y折促销审核期间不可进行编辑！');
                }
                if($xydiscountInfo['xydiscount_status'] =='agree' ){
                    throw new \LogicException('已通过X件Y折促销审核不可进行编辑！');
                }
                if($xydiscountInfo['xydiscount_status'] =='cancel' ){
                    throw new \LogicException('已取消X件Y折促销不可进行编辑！');
                }
            }
        }
        else
       ...
       /* if( $data['limit_number']<1)
        ...
        if(app::get('sysconf')->getConf('shop.promotion.examine')){
            $aResult['xydiscount_status'] = 'non-reviewed';
        }else{
            $aResult['xydiscount_status'] = 'agree';
        }
        return $aResult;
    }
