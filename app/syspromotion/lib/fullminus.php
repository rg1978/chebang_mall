    ...
    private function __preareData($data) {
        ...
        if($data['fullminus_id'])
        {
            ...
            if(!app::get('sysconf')->getConf('shop.promotion.examine')){
                ...
                if($fullminusInfo['fullminus_status'] =='pending'){
                    throw new \LogicException('满减促销审核期间不可进行编辑！');
                }
                if($fullminusInfo['fullminus_status'] =='agree' ){
                    throw new \LogicException('已通过满减促销审核不可进行编辑！');
                }
                if($fullminusInfo['fullminus_status'] =='cancel' ){
                    throw new \LogicException('已取消满减促销不可进行编辑！');
                }
            }
        }
        else
        ...
        if(app::get('sysconf')->getConf('shop.promotion.examine')){
            $aResult['fullminus_status'] = 'non-reviewed';
        }else{
            $aResult['fullminus_status'] = 'agree';
        }
