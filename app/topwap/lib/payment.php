...
class topwap_payment{
    ...
    public function getPaymentId($filter)
    {
        ...
        $tradeParams = array(
            ...
            'tid' => $tids,
            'fields' => 'tid,payment,points_fee,user_id,status',
        );
        //获取需要支付的订单并检测其有效性
        $tradeList = app::get('topwap')->rpcCall('trade.get.list',$tradeParams);
        ...
        if($countid != $count)
        {
            throw new \LogicException(app::get('topwap')->_("支付失败，提交的订单数据有误"));
            return false;
        }
        ...
        foreach($tradeList as $key=>$value)
        {
            ...
            {
                throw new \LogicException(app::get('topwap')->_($value['tid']." 订单已被支付,请重新选择要支付订单"));
                return false;
            }
        ...
        try
        {
            $paymentId = app::get('topwap')->rpcCall('payment.bill.create',$payment);
        }
        catch(Exception $e)
        {
            throw $e;
