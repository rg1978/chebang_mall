...
class ectools_api_getPaymentBill{
    ...
    public function getInfo($params)
    {
        ...
        if($params['status'])
        {
            $filterBill['status|in'] = $filter['status|in'] = explode(',',$params['status']);
        }

        if($filterBill)
        {
            $objMdlTradePaybill = app::get('ectools')->model('trade_paybill');
            $billList = $objMdlTradePaybill->getList('payment_id,tid,payment,status,paybill_id,payed_time',$filterBill);
            if($billList)
            {
