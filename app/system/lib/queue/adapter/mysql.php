...
class system_queue_adapter_mysql implements system_interface_queue_adapter{
    ...
    public function later($delay, $queue_data = '', $queue_name = null)
    {
        return $this->publish($queue_name, $queue_data);
    }
    ...
    public function get($queue_name)
    {
        if (($row = $this->__model->get($queue_name)))
        {
            $queue_data = array(
                ...
                'worker' => $row['worker']);
            return new system_queue_message_mysql($this, $queue_name, $queue_data);
        }
        return false;
    }
     ...
     * @return mixed 队列任务数据
     */
    public function ack($queueName, $queue_id)
    {
