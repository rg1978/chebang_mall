...
class system_queue_consumer_proc_thread {
     ...
     * @return void
     */
    function __construct($queue_name,$max_exec_time,$phpExec='') {
        ...
        if ($phpExec){
            ...
            {
                $executable = dirname(ini_get('extension_dir')).'/php.exe';
            }
            $executable = file_exists($executable) ? $executable : 'php';
            $executable = str_replace('\\','/',$executable);
            $scriptDir = str_replace('\\','/',SCRIPT_DIR);
        }else{
            $executable = PHP_BINDIR.'/php';
            $executable = file_exists($executable) ? $executable : 'php';
        }

        $scriptDir = $scriptDir ? $scriptDir : SCRIPT_DIR;
        $script = $scriptDir."/queue/queuescript.php";
        ...
        echo $executable." ".$script." ".$queue_name." ".$max_exec_time."\n";
