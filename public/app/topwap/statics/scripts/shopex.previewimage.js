(function($, window) {
	...
	var template = '<div id="{{id}}" class="shopex-slider shopex-preview-image shopex-fullscreen"><div class="shopex-preview-header">{{header}}</div><div class="shopex-slider-group"></div><div class="shopex-preview-footer shopex-hidden">{{footer}}</div><div class="shopex-preview-loading"><span class="shopex-spinner shopex-spinner-white"></span></div></div>';
	var itemTemplate = '<div class="shopex-slider-item shopex-zoom-wrapper {{className}}"><div class="shopex-zoom-scroller"><img src="{{src}}" data-preview-lazyload="{{lazyload}}" style="{{style}}" class="shopex-zoom"></div></div>';
	var defaultGroupName = '__DEFAULT';
	var div = document.createElement('div');
	var imgId = 0;
	var PreviewImage = function(options) {
		this.options = $.extend(true, {
			id: '__shopex_PREVIEWIMAGE',
			zoom: true,
			header: '<span class="shopex-preview-indicator"></span>',
			footer: ''
		}, options || {});
		this.init();
		this.initEvent();
	...
	proto.initEvent = function() {
		...
		this.element.addEventListener('slide', function(e) {
			if (self.options.zoom) {
				var lastZoomerEl = self.element.querySelector('.shopex-zoom-wrapper:nth-child(' + (self.lastIndex + 1) + ')');
				if (lastZoomerEl) {
	...
	proto.addImage = function(img) {
		...
		if (img.__shopex_img_data && img.__shopex_img_data.src === src) { //已缓存且图片未变化
			this.groups[group].push(img.__shopex_img_data);
		} else {
	...
	proto.open = function(index, group) {
		...
		if (typeof index === "number") {
			...
			group = group || defaultGroupName;
			this.addImages(group, index); //刷新当前group
			this.openByGroup(this.groups[group].indexOf(index.__shopex_img_data), group);
		}
	};
