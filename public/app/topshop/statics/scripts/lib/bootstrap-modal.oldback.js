	...
	Modal.prototype = {
		...
		layout: function () {
			...
			if (this.options.width){
				...
				var that = this;
				this.$dialog.css('margin-left', function () {
			...
			var modalOverflow = $(window).height() - 10 < this.$dialog.height();

			if (modalOverflow || this.options.modalOverflow) {
				this.$element.addClass('modal-overflow');
				this.$dialog.css('margin-top', 0);
			} else {
				this.$element.removeClass('modal-overflow');
				this.$dialog.css('margin-top', 0 - this.$dialog.height() / 2)
			}
		},
